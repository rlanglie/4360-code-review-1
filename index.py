...
from exceptions import BankingException, LoginAttemptAmtException

def main():
    try: #15 wrapped with try statement to allow exceptions to raise properly
        message = Message()
        ui = UserInterface()
        auth = Auth()
        ...
        while login_attempts < 3:
            ...
            if not user:
                ...
                break
            if login_attempts >= 3: #9 added login attempt check
                raise LoginAttemptAmtException
        ...
        while True:
            ...
            if not success:
                input("Press Enter to continue...")  #8 added pause before clearing the screen to allow exceptions to display
